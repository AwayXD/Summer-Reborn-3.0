...
import summer.cheat.cheats.render.CustomGlint;
        ...
        if (!Config.isCustomItems() || CustomItems.isUseGlint()) {
            if (!Config.isShaders() || !Shaders.isShadowPass) {
                ...
                for (int i = 0; i < 2; ++i) {
                    ...
                    float f2 = 0.76F;
                    if (CheatManager.getInstance(CustomGlint.class).isToggled()) {
                        Color c = Color.getHSBColor(CustomGlint.armorHue.getValFloat(), 1.0F, 1.0F);
                        float red = c.getRed() / 255.0F;
                        float blue = c.getBlue() / 255.0F;
                        float green = c.getGreen() / 255.0F;
                        if (CustomGlint.rainbow.getValBoolean()) {
