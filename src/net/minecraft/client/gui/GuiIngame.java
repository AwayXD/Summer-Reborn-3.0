...
import summer.cheat.cheats.render.Crosshair;
import summer.cheat.cheats.render.HUD;
import summer.cheat.eventsystem.events.render.EventRender2D;
import summer.cheat.eventsystem.events.render.EventRenderGui;
...
public class GuiIngame extends Gui {
    ...
    public void renderGameOverlay(float partialTicks) {
        ...
        if (this.showCrosshair() && this.mc.gameSettings.thirdPersonView < 1
                && !CheatManager.getInstance(Crosshair.class).isToggled()) {
        ...
        EventRenderGui erg = new EventRenderGui();
        erg.call();
    ...
    protected void renderTooltip(ScaledResolution sr, float partialTicks) {
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
            Color c = Color.getHSBColor(HUD.hotbarHue.getValFloat(), 1.0F, 1.0F);
            ...
            if (HUD.hotbar.getValBoolean()) {
                Gui.drawRect(i - 91 - 1 + entityplayer.inventory.currentItem * 20 + 1,
                        sr.getScaledHeight() - 22, sr.getScaledWidth() - i
                                - 91 - 1 + (entityplayer.inventory.currentItem * 20) + 22,
                        sr.getScaledHeight(), color);
                RenderUtils.drawBorderedRect(i - 91 - 1 + entityplayer.inventory.currentItem * 20 + 1,
                        sr.getScaledHeight() - 22, sr.getScaledWidth() - i
                                - 91 - 1 + (entityplayer.inventory.currentItem * 20) + 22,
                        sr.getScaledHeight(), 2, 0x80000000, 0x10000000);
                RenderUtils.drawBorderedRect(0, sr.getScaledHeight() - 22,
                        sr.getScaledWidth(), sr.getScaledHeight(), 2,
                        0x80000000, 0x90000000);
                if (HUD.hotbarRainbow.getValBoolean()) {
                    Gui.drawHorizontalLine(0, sr.getScaledWidth(), sr.getScaledHeight() - 23, Chams.getRainbow(10000, -15 * yText));
                } else {
                    Gui.drawHorizontalLine(0, sr.getScaledWidth(), sr.getScaledHeight() - 23, color);
                }
            } else {
    ...
    private void renderScoreboard(ScoreObjective p_180475_1_, ScaledResolution p_180475_2_) {
        ...
        int amount = summer.cheat.cheats.render.Scoreboard.height.getValInt();
        boolean hide = summer.cheat.cheats.render.Scoreboard.hide.getValBoolean();
        int j1 = arraylist1.size() * this.getFontRenderer().FONT_HEIGHT;
        int k1 = p_180475_2_.getScaledHeight() / 2 + j1 / 1 + amount;
        if (hide) {
            k1 = p_180475_2_.getScaledHeight() / 2 + j1 / 1 + 300;
        }
        byte b0 = 3;
        ...
        for (Object score1 : arraylist1) {
            ...
            if (summer.cheat.cheats.render.Scoreboard.drawShadow.getValBoolean()) {
                if (summer.cheat.cheats.render.Scoreboard.transparent.getValBoolean()) {
                    if (summer.cheat.cheats.render.Scoreboard.hideNumbers.getValBoolean()) {
//                    drawRect(j + 5, l, i1, l + this.getFontRenderer().FONT_HEIGHT, 1342177280);
                        this.getFontRenderer().drawStringWithShadow(s1, j + 8, l, 553648127);
                    } else {
//                    drawRect(j - 2, l, i1, l + this.getFontRenderer().FONT_HEIGHT, 1342177280);
                ...
                if (summer.cheat.cheats.render.Scoreboard.transparent.getValBoolean()) {
                    if (summer.cheat.cheats.render.Scoreboard.hideNumbers.getValBoolean()) {
//                    drawRect(j + 5, l, i1, l + this.getFontRenderer().FONT_HEIGHT, 1342177280);
                        this.getFontRenderer().drawString(s1, j + 8, l, 553648127);
                    } else {
//                    drawRect(j - 2, l, i1, l + this.getFontRenderer().FONT_HEIGHT, 1342177280);
            ...
            if (k == arraylist1.size()) {
                ...
                if (summer.cheat.cheats.render.Scoreboard.drawShadow.getValBoolean()) {
                    if (summer.cheat.cheats.render.Scoreboard.transparent.getValBoolean()) {
                        if (summer.cheat.cheats.render.Scoreboard.hideNumbers.getValBoolean()) {
                            this.getFontRenderer().drawStringWithShadow(s3, j + 105 / 2 - this.getFontRenderer().getStringWidth(s3) / 2,
                                    l - this.getFontRenderer().FONT_HEIGHT, 553648127);
                        } else {
                            this.getFontRenderer().drawStringWithShadow(s3, j + i / 2 - this.getFontRenderer().getStringWidth(s3) / 2,
                                    l - this.getFontRenderer().FONT_HEIGHT, 553648127);
                        }
                    } else {
                        if (summer.cheat.cheats.render.Scoreboard.hideNumbers.getValBoolean()) {
                            drawRect(j + 5, l - this.getFontRenderer().FONT_HEIGHT - 1, i1, l - 1, 1342177280);
                            drawRect(j + 5, l - 1, i1, l, 1342177280);
                            this.getFontRenderer().drawStringWithShadow(s3, j + 105 / 2 - this.getFontRenderer().getStringWidth(s3) / 2,
                                    l - this.getFontRenderer().FONT_HEIGHT, 553648127);
                        } else {
                            drawRect(j - 2, l - this.getFontRenderer().FONT_HEIGHT - 1, i1, l - 1, 1342177280);
                    ...
                    if (summer.cheat.cheats.render.Scoreboard.transparent.getValBoolean()) {
                        if (summer.cheat.cheats.render.Scoreboard.hideNumbers.getValBoolean()) {
                            this.getFontRenderer().drawString(s3, j + 105 / 2 - this.getFontRenderer().getStringWidth(s3) / 2,
                                    l - this.getFontRenderer().FONT_HEIGHT, 553648127);
                        } else {
