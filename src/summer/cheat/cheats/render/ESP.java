...
public class ESP extends Cheats {
        ...
        for (EntityPlayer player : entityPosMap.keySet()) {
            ...
            if (nametags.getValBoolean()) {
                ...
                float scale = Math.max(minScale,
                        Math.min(1.0F, 1.0F - (Minecraft.getMinecraft().thePlayer.getDistanceToEntity(player) / 100.0F)));
                float yOff = Math.max(0.0F,
                        Math.min(1.0F, Minecraft.getMinecraft().thePlayer.getDistanceToEntity(player) / 12.0F));
    ...
    private void drawSkeleton(final EventRender3D event, final EntityPlayer e) {
        final Color color = new Color(
                e.getName().equalsIgnoreCase(Minecraft.getMinecraft().thePlayer.getName()) ? -6684775 : new Color(16775672).getRGB());
        if (!e.isInvisible()) {
