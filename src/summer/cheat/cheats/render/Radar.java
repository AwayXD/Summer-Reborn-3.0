            ...
            for (Entity entity : Minecraft.theWorld.loadedEntityList) {
                if (qualifies(entity)) {
                    ...
                    double dx = RenderUtils.lerp(entity.prevPosX, entity.posX, event.getTicks())
                            - RenderUtils.lerp(Minecraft.getMinecraft().thePlayer.prevPosX, Minecraft.getMinecraft().thePlayer.posX,
                            event.getTicks());
                    // Z difference
                    double dz = RenderUtils.lerp(entity.prevPosZ, entity.posZ, event.getTicks())
                            - RenderUtils.lerp(Minecraft.getMinecraft().thePlayer.prevPosZ, Minecraft.getMinecraft().thePlayer.posZ,
                            event.getTicks());
                    ...
                    if ((dx * dx + dz * dz) <= (maxDist * maxDist)) {
                        ...
                        double[] vector = getLookVector(
                                RotationUtils.getRotations(entity)[0] - (float) RenderUtils.lerp(Minecraft.getMinecraft().thePlayer.prevRotationYawHead, Minecraft.getMinecraft().thePlayer.rotationYawHead, event.getTicks()));
