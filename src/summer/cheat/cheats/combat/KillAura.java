...
//        }
        ...
        if (target != null && eu.isPre()) {
            this.rotate(eu);
            double distance = Minecraft.getMinecraft().thePlayer.getDistanceToEntity(target);
            ...
            if (distance <= blockRange.getValDouble() && autoblock.getValBoolean() && Minecraft.getMinecraft().thePlayer.getCurrentEquippedItem() != null
                    && Minecraft.getMinecraft().thePlayer.getCurrentEquippedItem().getItem() instanceof ItemSword
                    && !autoblockType.getValString().equalsIgnoreCase("Vanilla")) {
                Minecraft.getMinecraft().thePlayer.setItemInUse(Minecraft.getMinecraft().thePlayer.getCurrentEquippedItem(),
                        Minecraft.getMinecraft().thePlayer.getCurrentEquippedItem().getMaxItemUseDuration());
            }
    ...
    private boolean isValid(Entity target) {
        ...
        if (target instanceof EntityPlayer && !players.getValBoolean()) {
            ...
            return false;
        } else if (Minecraft.getMinecraft().thePlayer.getDistanceToEntity(target) > Math.max(rangeValue.getValDouble(), blockRange.getValDouble())) {
            return false;
        }
    ...
    public static boolean attack() {
        ...
        Timer.timerSpeed = 1f;
        Minecraft.getMinecraft().thePlayer.sendQueue.addToSendQueue(new C02PacketUseEntity(target, Action.ATTACK));
        Timer.timerSpeed = 1f;
        return true;
    }
    ...
    private void block() {
        if (autoblockType.getValString().equalsIgnoreCase("Watchdog")) {
            double value = -1;
            Minecraft.getMinecraft().thePlayer.sendQueue.addToSendQueue(new C08PacketPlayerBlockPlacement(new BlockPos(value, value, value), 255, mc.thePlayer.inventory.getCurrentItem(), 0, 0, 0));
        }
    }
}
