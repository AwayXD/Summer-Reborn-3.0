...
public class AutoArmor extends Cheats {
    ...
    public void getBestArmor() {
        ...
        if (drop.getValBoolean())
            for (int j = 9; j < 45; j++) {
                if (Minecraft.getMinecraft().thePlayer.inventoryContainer.getSlot(j).getHasStack()) {
                    ItemStack is = Minecraft.getMinecraft().thePlayer.inventoryContainer.getSlot(j).getStack();
    ...
    public void shiftClick(int slot) {
        this.mc.playerController.windowClick(Minecraft.getMinecraft().thePlayer.inventoryContainer.windowId, slot, 0, 1,
                (EntityPlayer) Minecraft.thePlayer);
    }
    ...
    public void drop(int slot) {
        if (drop.getValBoolean()
                ...
                && !this.dropping) {
            this.mc.playerController.windowClick(Minecraft.getMinecraft().thePlayer.inventoryContainer.windowId, slot, 0, 0,
                    (EntityPlayer) Minecraft.thePlayer);
            this.dropping = true;
            this.dropTimer.reset();
        }
