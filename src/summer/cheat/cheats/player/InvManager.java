...
public class InvManager extends Cheats {
    ...
    public void onPre(final EventUpdate event) {
        ...
        if (event.isPre()) {
            ...
            if (Minecraft.thePlayer != null && !dropping
                ...
                for (int i = 9; i < 45; ++i) {
                    if (Minecraft.getMinecraft().thePlayer.inventoryContainer.getSlot(i).getHasStack()) {
                        ...
                        if (this.isBad(is, i) && !(is.getItem() instanceof ItemArmor)
                                && is != Minecraft.getMinecraft().thePlayer.getCurrentEquippedItem()) {
                            if (invcleaner) {
                                this.mc.playerController.windowClick(Minecraft.getMinecraft().thePlayer.inventoryContainer.windowId, i,
                            ...
                            if (weaponSlot < 10 && is.getItem() instanceof ItemSword && isBestWeapon(is)
                                ...
                                mc.playerController.windowClick(Minecraft.getMinecraft().thePlayer.inventoryContainer.windowId, i, weaponSlot,
                ...
                this.mc.playerController.windowClick(Minecraft.getMinecraft().thePlayer.inventoryContainer.windowId, -999, 0, 0,
                        Minecraft.thePlayer);
                timer.reset();
                dropping = false;
            }
    ...
    private boolean isBad(final ItemStack stack, int slot) {
        ...
        if ((slot == weaponSlot && isBestWeapon(Minecraft.getMinecraft().thePlayer.inventoryContainer.getSlot(weaponSlot).getStack()))) {
            return false;
        }
    ...
    private List<ItemStack> getBest() {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            for (final ItemStack itemStack : Minecraft.getMinecraft().thePlayer.inventory.armorInventory) {
                if (itemStack != null) {
    ...
    private ItemStack findBestArmor(final int itemSlot) {
        ...
        for (int slot = 0; slot < 36; ++slot) {
            final ItemStack itemStack = Minecraft.getMinecraft().thePlayer.inventory.mainInventory[slot];
            if (itemStack != null) {
