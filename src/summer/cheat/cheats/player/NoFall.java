...
public class NoFall extends Cheats {
    ...
    public void onEvent(EventUpdate eventMotion) {
        ...
        if (!Minecraft.getMinecraft().thePlayer.isSpectator() && !Minecraft.getMinecraft().thePlayer.capabilities.allowFlying && Minecraft.getMinecraft().thePlayer.fallDistance > 3) {
            Minecraft.getMinecraft().thePlayer.sendQueue.addToSendQueue(new C03PacketPlayer(ThreadLocalRandom.current().nextBoolean()));
        }
    }
}