...
//        Summer.instance.sm.Property(extend = new Setting("Extend", this, 0.1, 0.1, 6, false));
    ...
    private boolean placeBlock(final BlockPos pos, final EnumFacing facing) {
        final Vec3 eyesPos = new Vec3(Minecraft.getMinecraft().thePlayer.posX, Minecraft.getMinecraft().thePlayer.posY + Minecraft.getMinecraft().thePlayer.getEyeHeight(),
                Minecraft.getMinecraft().thePlayer.posZ);

        if (mc.playerController.onPlayerRightClick(Minecraft.thePlayer, Minecraft.theWorld, Minecraft.getMinecraft().thePlayer.getHeldItem(), pos, facing,
    ...
    private BlockCache grab() {
        ...
        if (Minecraft.getMinecraft().thePlayer.isMoving() && !mc.gameSettings.keyBindJump.getIsKeyPressed()) {
            for (double n2 = 0.08D, n3 = 0.0; n3 <= n2; n3 += n2 / (Math.floor(n2) + 1.0)) {
                final BlockPos blockPos2 = new BlockPos(
                        Minecraft.getMinecraft().thePlayer.posX - MathHelper.sin(ScaffoldUtils.clampRotation()) * n3,
                        Minecraft.getMinecraft().thePlayer.posY - 1.0,
                        Minecraft.getMinecraft().thePlayer.posZ + MathHelper.cos(ScaffoldUtils.clampRotation()) * n3);
    ...
    public void onUpdate(EventUpdate event) {
        ...
        if (event.isPre()) {
            ...
            if (mc.gameSettings.keyBindJump.isKeyDown() && !Minecraft.getMinecraft().thePlayer.isMoving()
                    && tower.getValBoolean()) {
                ...
                Minecraft.getMinecraft().thePlayer.isJumping = false;
                Minecraft.getMinecraft().thePlayer.setJumping(false);
            ...
            final int currentSlot = Minecraft.getMinecraft().thePlayer.inventory.currentItem;
            ...
            if (timer.hasReached(time)) {
                ...
                if (this.placeBlock(blockCache.position, blockCache.facing)) {
                    ...
                    for (int i = 0; i < 9; i++) {
                        if (Minecraft.getMinecraft().thePlayer.inventory.mainInventory[i] == Minecraft.getMinecraft().thePlayer.inventory.mainInventory[currentSlot]) {
                            exists = true;
