...
public class AutoPot extends Cheats {
    ...
    public void update(EventUpdate e) {
        if (CheatManager.getInstance(Flight.class).isToggled() || Minecraft.getMinecraft().thePlayer.isOnLadder())
            return;
        EventUpdate eu = e;
        if (eu.isPre() && this.timer.hasReached(delay.getValFloat())) {
            ...
            if (eu.getLastPitch() == 85.0F) {
                Minecraft.getMinecraft().thePlayer.sendQueue.addToSendQueue(new C09PacketHeldItemChange(toThrow));
    ...
    private ItemStack getStackToThrow2() {
        for (int k = 0; k < 9; k++) {
            ItemStack item = Minecraft.getMinecraft().thePlayer.inventory.mainInventory[k];
    ...
    private boolean isValid(ItemStack stack) {
        ...
        for (PotionEffect pe : effects) {
            if (Minecraft.getMinecraft().thePlayer.isPotionActive(pe.getPotionID())) {
                containsBad = true;
                continue;
            }
            if (pe.getPotionID() == Potion.regeneration.id || pe.getPotionID() == Potion.heal.id) {
                ...
                if (Minecraft.getMinecraft().thePlayer.getHealth() / Minecraft.getMinecraft().thePlayer.getMaxHealth() > healPercent.getValFloat() / 100.0F)
                    containsBad = true;
                continue;
            }
