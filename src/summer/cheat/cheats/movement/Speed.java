...
public class Speed extends Cheats {
    ...
    public void onUpdate(final EventMotion eventMove) {
        ...
        switch (this.stage) {
            ...
            case 2:
                ...
                if (Minecraft.getMinecraft().thePlayer.isMoving() && Minecraft.getMinecraft().thePlayer.onGround) {
                    ...
                    eventMove.setY(Minecraft.getMinecraft().thePlayer.motionY = motionY);
                    this.hops++;
            ...
            case 3:
                ...
                n = Minecraft.getMinecraft().thePlayer.isPotionActive(Potion.moveSpeed) ? (Minecraft.getMinecraft().thePlayer.isPotionActive(Potion.jump) ? 0.54D : 0.655D) : 0.7025D;
                lastDist2 = this.lastDist;
                thePlayer = Minecraft.thePlayer;
                this.moveSpeed = lastDist - n * (lastDist2 - EntityPlayerSP.getBaseMoveSpeed());
                break;
            default:
                if ((mc.theWorld.getCollidingBoundingBoxes((Entity)Minecraft.thePlayer, Minecraft.getMinecraft().thePlayer.getEntityBoundingBox().offset(0.0D, Minecraft.getMinecraft().thePlayer.motionY, 0.0D)).size() > 0 || Minecraft.getMinecraft().thePlayer.isCollidedVertically) && this.stage > 0)
    ...
    public static boolean isNotCollidingBelow(double paramDouble) {
        if (!Minecraft.theWorld.getCollidingBoundingBoxes(Minecraft.thePlayer, Minecraft.getMinecraft().thePlayer.getEntityBoundingBox().offset(0.0D, -paramDouble, 0.0D)).isEmpty()) {
            return true;
        }
        return false;
    }
